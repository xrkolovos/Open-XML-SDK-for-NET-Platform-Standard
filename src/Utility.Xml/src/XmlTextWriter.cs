...
namespace Utility.Xml
{
    ...
    public class XmlTextWriter : XmlWriter
    {
        ...
        struct TagInfo
        {
            ...
            public NamespaceState defaultNsState;
            public XmlSpace xmlSpace;
            public string xmlLang;
            public int prevNsTop;
            public int prefixCount;
            public bool mixed; // whether to pretty print the contents of this element.

            public void Init(int nsTop)
            {
                name = null;
        ...
        struct Namespace
        {
            ...
            public void Set(string prefix, string ns, bool declared)
            {
                this.prefix = prefix;
                this.ns = ns;
        ...
        public bool Namespaces
        {
            ...
            set
            {
                if (this.currentState != State.Start)
                    throw new Exception("throw new InvalidOperationException(SR.Xml_NotInWriteState);");
        ...
        public int Indentation
        {
            ...
            {
                if (value < 0)
                    throw new Exception("throw new ArgumentException(SR.Xml_InvalidIndentation);");
        ...
        public char QuoteChar
        {
            ...
            {
                if (value != '"' && value != '\'') {
                    throw new Exception("throw new ArgumentException(SR.Xml_InvalidQuote);");
            ...
            try {
                ...
                if (this.currentState != State.Epilog) {
                    if (this.currentState == State.Closed) {
                        throw new Exception("throw new ArgumentException(SR.Xml_ClosedOrError);");
                        //throw new ArgumentException(SR.Xml_ClosedOrError);
                    } else {
                        throw new Exception("throw new ArgumentException(SR.Xml_NoRoot);");
                ...
                if (this.namespaces) {
                    ...
                    if (ns == null) {
                        ...
                        if (!string.IsNullOrEmpty(prefix) && (LookupNamespace(prefix) == -1)) {
                            throw new Exception("throw new ArgumentException(SR.Xml_UndefPrefix);");
                    ...
                    if (!string.IsNullOrEmpty(ns) || !string.IsNullOrEmpty(prefix)) {
                        throw new Exception("throw new ArgumentException(SR.Xml_NoNamespaces);");
        ...
        {
            publicWriteEndElement(false);
        }

        // Closes one element and pops the corresponding namespace scope.
        public override void WriteFullEndElement()
        {
            publicWriteEndElement(true);
        }
                ...
                if (this.namespaces) {
                    ...
                    if (prefix == "xml") {
                        ...
                        if (XmlConst.ReservedNsXmlNs != ns && ns != null) {
                            throw new Exception("throw new ArgumentException(SR.Xml_XmlnsBelongsToReservedNs);");
                        ...
                        if (XmlConst.ReservedNsXmlNs != ns && ns != null) {
                            ...
                            throw new ArgumentException(SR.GetResourceString("Xml_XmlnsBelongsToReservedNs"));
                        }
                        this.specialAttr = SpecialAttr.XmlNs;
                        this.prefixForXmlNs = null;
                    } else {
                        if (ns == null) {
                            ...
                            if (prefix != null && (LookupNamespace(prefix) == -1)) {
                                throw new ArgumentException(SR.GetResourceString("Xml_UndefPrefix"));
                            }
                        } else if (ns.Length == 0) {
                    ...
                    if (!string.IsNullOrEmpty(ns) || !string.IsNullOrEmpty(prefix)) {
                        throw new ArgumentException(SR.GetResourceString("Xml_NoNamespaces"));
                    }
        ...
        public override void WriteCData(string text)
        {
            try {
                ...
                if (null != text && text.IndexOf("]]>", StringComparison.Ordinal) >= 0) {
                    throw new ArgumentException(SR.GetResourceString("Xml_InvalidCDataChars"));
                }
        ...
        public override void WriteComment(string text)
        {
            try {
                if (null != text && (text.IndexOf("--", StringComparison.Ordinal) >= 0 || (text.Length != 0 && text[text.Length - 1] == '-'))) {
                    throw new ArgumentException(SR.GetResourceString("Xml_InvalidCommentChars"));
                }
        ...
        public override void WriteProcessingInstruction(string name, string text)
        {
            try {
                if (null != text && text.IndexOf("?>", StringComparison.Ordinal) >= 0) {
                    throw new ArgumentException(SR.GetResourceString("Xml_InvalidPiChars"));
                }
                if (String.Equals(name, "xml", StringComparison.OrdinalIgnoreCase) && this.stateTable == stateTableDocument) {
                    throw new ArgumentException(SR.GetResourceString("Xml_DupXmlDecl"));
                }
                AutoComplete(Token.PI);
                publicWriteProcessingInstruction(name, text);
            } catch {
                currentState = State.Error;
                throw;
            }
        ...
        public override void WriteWhitespace(string ws)
        {
            try {
                ...
                if (!xmlCharType.IsOnlyWhitespace(ws)) {
                    throw new ArgumentException(SR.GetResourceString("Xml_NonWhitespace"));
                }
        ...
        public override void WriteBinHex(byte[] buffer, int index, int count)
        {
            try {
                ...
                var rtn = BinHexEncoder.Encode(buffer, index, count);
                this.WriteString(rtn);
            } catch {
                currentState = State.Error;
                throw;
            }
        ...
        public override void WriteQualifiedName(string localName, string ns)
        {
            try {
                    ...
                    if (!string.IsNullOrEmpty(ns) && ns != stack[top].defaultNs) {
                        ...
                        if (prefix == null) {
                            if (this.currentState != State.Attribute) {
                                throw new ArgumentException(SR.Format(SR.GetResourceString("Xml_UndefNamespace"), ns));
                            }
                            prefix = GeneratePrefix(); // need a prefix if
                            PushNamespace(prefix, ns, false);
                        }
                        if (prefix.Length != 0) {
                            publicWriteName(prefix, true);
                            textWriter.Write(':');
                        }
                    }
                ...
                publicWriteName(localName, true);
            } catch {
                currentState = State.Error;
                throw;
            }
        ...
        public override string LookupPrefix(string ns)
        {
            if (string.IsNullOrEmpty(ns)) {
                throw new ArgumentException(SR.GetResourceString("Xml_EmptyName"));
            }
        ...
        public override void WriteNmToken(string name)
        {
            try {
                ...
                if (!ValidateNames.IsNmtokenNoNamespaces(name)) {
                    throw new ArgumentException(SR.Format(SR.GetResourceString("Xml_EmptyNameSR.Xml_InvalidNameChars"), name));
                }
                textWriter.Write(name);
            } catch {
                currentState = State.Error;
        ...
        void StartDocument(int standalone)
        {
            try {
                if (this.currentState != State.Start) {
                    throw new InvalidOperationException(SR.GetResourceString("Xml_NotTheFirst"));
                }
                ...
                publicWriteProcessingInstruction("xml", bufBld.ToString());
            } catch {
                currentState = State.Error;
                throw;
            }
        ...
        {
            if (this.currentState == State.Closed) {
                throw new InvalidOperationException(SR.GetResourceString("Xml_Closed"));
            } else if (this.currentState == State.Error) {
                throw new InvalidOperationException(SR.Format(SR.GetResourceString("Xml_WrongToken"), tokenName[(int)token], stateName[(int)State.Error]));
            }

            State newState = this.stateTable[(int)token * 8 + (int)this.currentState];
            if (newState == State.Error) {
                throw new InvalidOperationException(SR.Format(SR.GetResourceString("Xml_WrongToken"), tokenName[(int)token], stateName[(int)this.currentState]));
            }
            ...
            switch (token) {
                ...
                default:
                    throw new InvalidOperationException(SR.GetResourceString("Xml_InvalidOperation"));
            }
        ...
        void publicWriteEndElement(bool longFormat)
        {
            try {
                if (top <= 0) {
                    throw new InvalidOperationException(SR.GetResourceString("Xml_NoStartTag"));
                }
        ...
        void PushNamespace(string prefix, string ns, bool declared)
        {
            if (XmlConst.ReservedNsXmlNs == ns) {
                throw new ArgumentException(SR.GetResourceString("Xml_CanNotBindToReservedNamespace"));
            }
            ...
            if (prefix == null) {
                ...
                if (prefix.Length != 0 && ns.Length == 0) {
                    throw new ArgumentException(SR.GetResourceString("Xml_PrefixForEmptyNs"));
                }
        ...
        void publicWriteProcessingInstruction(string name, string text)
        {
            textWriter.Write("<?");
            ValidateName(name, false);
            textWriter.Write(name);
        ...
        void publicWriteName(string name, bool isNCName)
        {
        ...
        {
            if (string.IsNullOrEmpty(name)) {
                throw new Exception("throw new ArgumentException(SR.Xml_EmptyName);");
            ...
            throw new ArgumentException(SR.Format(SR.GetResourceString("Xml_InvalidNameChars"), name));
        }
            ...
            switch (this.specialAttr) {
                ...
                case SpecialAttr.XmlSpace:
                    ...
                    if (value == "default") {
                        ...
                        throw new ArgumentException(SR.Format(SR.GetResourceString("Xml_InvalidXmlSpace"), value));
                    }
                    break;
                ...
                if (
                   ...
                   (prefix[2] == 'l' || prefix[2] == 'L')
                    ...
                    if (XmlConst.ReservedNsXml != ns) {
                        throw new ArgumentException(SR.GetResourceString("Xml_InvalidPrefix"));
                    }
                }
            }
        }
